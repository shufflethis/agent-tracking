...
export default function FindingEditor({ domain, runs, fixes, initial, lang }: { domain: string; runs: string[]; fixes: string[]; initial?: Finding; lang: "de" | "en" }) {
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      const response = await fetch(`/api/findings/${encodeURIComponent(domain)}`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ findingId: initial?.findingId ?? crypto.randomUUID(), taskRunId, fixId: fixId || null, evidenceRefs: initial?.evidenceRefs ?? [`run:${taskRunId}`], category, description, assignee: assignee || null, status, correction: correction || null, retestRunId: retestRunId || null }) });
  ...
  return <form onSubmit={save} style={{ display: "grid", gap: 10, maxWidth: 700 }}>
    ...
    <label>{lang === "de" ? "Kategorie" : "Category"}<select value={category} onChange={(event) => setCategory(event.target.value)}>{categories.map((c) => <option key={c}>{c}</option>)}</select></label>
