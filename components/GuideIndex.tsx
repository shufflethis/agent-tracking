      ...
      <div className="grid2" style={{ gap: 18, marginBottom: 42 }}>
        {featured.map((g) => (
          <Link key={g.slug} href={`${base}/${g.slug}`} className="card" style={{ padding: 24, display: "block" }}>
            <p className="eyebrow" style={{ marginTop: 0 }}>{lang === "de" ? "Ausführlicher Leitfaden" : "In-depth guide"}</p>
            <h2 style={{ fontSize: 23, marginBottom: 8 }}>{g.question}</h2>
            <p style={{ margin: 0, fontSize: 14.5, color: "var(--ink-2)", lineHeight: 1.55 }}>{g.summary}</p>
          </Link>
        ))}
      </div>
      <h2 style={{ marginBottom: 20 }}>{lang === "de" ? "Fragen im Detail" : "Specific questions"}</h2>
