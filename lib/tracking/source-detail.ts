...
export function sourceDetail(domain: string, key: string, days: number, now = Date.now()) {
  ...
  return { kind, id, label: (kind === "fetch" ? AGENT_LABELS[id] : REFERRER_LABELS[id]) ?? id, days: window,
    ...
    totals, timeline, rawCount: raw.reduce((n,r) => n+r.count,0), rawRetentionDays: RAW_RETENTION_DAYS,
